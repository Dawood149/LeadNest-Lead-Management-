      ...
      <nav className="m-2">
        <div className="bg-gray-800  flex relative h-15 rounded p-2">
          <div className="absolute right-50 flex gap-15">
            ...
            <div>
              <button
                className="cursor-pointer transition duration-300 ease-in-out hover:scale-120 "
              ...
              {notificationPanelOpen && (
                <div className="bg-white text-black left-0 absolute mt-3 w-100 h-100 bg-white-700 rounded-lg shadow-xl py-1 z-10">
                  <button className=" cursor-pointer block w-full text-left px-4 py-2 text-sm  hover:bg-zinc-200 -b-1">
                    Notification
                  </button>
          ...
          <div className="absolute right-0 text-white p-3 ">
            <button
              onClick={() => setLogoutDropdown(!isLogoutDropdownOpen)}
              className="flex items-center space-x-2 focus:outline-none cursor-pointer"
            >
              <img src={userImg} alt="User" className="w-9 h-8 rounded-full" />
              <span>{name}</span>
            </button>

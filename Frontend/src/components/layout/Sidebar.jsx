...
const { name, role } = useSelector((state) => state.user);
  ...
  return (
    <>
      <div
        on={toggleSidebar}
        className={`h-full bg-gray-800 m-2 rounded transition-all duration-300 ease-in-out ${
          isOpen ? "w-64" : "w-20"
        }`}
      >
          ...
          <ul className={`space-y-7 ${isOpen ? "w-full px-4" : "w-full px-2"}`}>
            ...
            <li className="transition duration-200 ease-in-out ">
              <NavLink
                to={"/dashboard"}
                className={({ isActive }) =>
                  `flex items-center gap-2 p-2 rounded transition duration-200 ease-in-out ${
                    isActive
